...
extension SiriMainNavigator {
    private func instantiateViewController<T: UIViewController>(withId identifier: String) -> T {
        guard let viewController = rootViewController.storyboard?.instantiateViewController(withIdentifier: identifier) as? T else {
            fatalError("view controller of type \(T.self) does not exist with id \(identifier)")
        }
