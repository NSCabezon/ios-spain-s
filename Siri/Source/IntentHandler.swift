            ...
            case .success(let loginOutput, let managerList):
                ...
                case .login:
                    guard let managers = managerList?.managers, let manager = managers.first else {
                        completion(CallToManagerIntentResponse(code: .callOkNoManager, userActivity: nil))
                        return
                    }
                ...
                case .notTokenForLogin:
                    completion(CallToManagerIntentResponse(code: .noToken, userActivity: nil))
                }
            case .failure:
                completion(CallToManagerIntentResponse(code: .failure, userActivity: nil))
            }
        }
    }
}
