...
final class TodayViewController: UIViewController, NCWidgetProviding {
    ...
    lazy var accountInfoView: WidgetAccountInfoView? = {
        return WidgetAccountInfoView.initFromNib()
    }()
    
    lazy var messageLabelView: WidgetMessageLabelView? = {
        return WidgetMessageLabelView.initFromNib()
    }()
    
    private var currentView: (UIView & Refreshable)?
    ...
    private func buildDeepLinks() {
        for view in linksStackView.arrangedSubviews {
            linksStackView.removeArrangedSubview(view)
...
extension TodayViewController {
    fileprivate struct Constants {
        static var compactSize: CGFloat { 110.0 }
    }
}
