...
protocol WidgetAccountsTransactionsSuperUseCaseDelegate: AnyObject {
    func finish(accountTransactions: [AccountTransaction])
}

protocol WidgetAccountsTransactionsSuperUseCaseProtocol: AnyObject {
...
class WidgetAccountsTransactionsSuperUseCase {
    ...
    private var operations: ThreadSafeProperty<[Int: Foundation.Operation & LoadAccountTransactionProtocol]> = ThreadSafeProperty([:])
