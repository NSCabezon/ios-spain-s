...
class KeychainAuthentication {
    ...
    var isTouchIdEnabled: Bool {
        guard let touchIdEnabled = touchIdData?.touchIDLoginEnabled else {
            return false
        }
...
private extension KeychainAuthentication {
    func returnBiometryType(_ biometryType: LABiometryType) -> BiometryType {
        switch biometryType {
        case .touchID:
            return .touchId
        case .faceID:
    ...
    func returnBiometryError(biometry: LABiometryType, error: BiometryError) -> BiometryType {
        switch biometry {
        case .touchID:
            return .error(biometry: .touchId, error: error)
        case .faceID:
            return .error(biometry: .faceId, error: error)
        case .none:
            return .error(biometry: .none, error: error)
        @unknown default:
            return .none
        }
    }
    
    func findOutBiometryType() -> BiometryType {
        var error: NSError?
        if context.canEvaluatePolicy(policy, error: &error) {
            if #available(iOS 11, *) {
                return returnBiometryType(context.biometryType)
            } else {
                return .touchId
            }
        }
        ...
        if #available(iOS 11, *) {
            return returnBiometryError(biometry: context.biometryType, error: BiometryError(laError: laError))
        }
    ...
    func returnBiometryTypeEntity(_ biometryType: LABiometryType) -> BiometryTypeEntity {
        switch biometryType {
        case .touchID:
            return .touchId
        case .faceID:
    ...
    func returnBiometryErrorEntity(biometry: LABiometryType, error: BiometryErrorEntity) -> BiometryTypeEntity {
        switch biometry {
        case .touchID:
            return .error(biometry: .touchId, error: error)
        case .faceID:
    ...
    func findOutBiometryTypeEntity() -> BiometryTypeEntity {
        ...
        if context.canEvaluatePolicy(policy, error: &error) {
            if #available(iOS 11, *) {
                return returnBiometryTypeEntity(context.biometryType)
            } else {
        ...
        if #available(iOS 11, *) {
            return returnBiometryErrorEntity(biometry: context.biometryType, error: BiometryErrorEntity(laError: laError))
        }
        let hasNoBiometry = UIDevice.current.hasNoBiometry
