...
//
//  Created by José María Jiménez Pérez on 22/6/21.
...
class AmountTextFieldFormatterNoDecimalsTests: XCTestCase {
    var sut: AmountTextFieldFormatter!
    var textField: UITextField!
    var inspector: MockChangeTestFieldDelegate!
    
    override func setUp() {
        sut = AmountTextFieldFormatter(maximumIntegerDigits: 12, maximumFractionDigits: 0)
        inspector = MockChangeTestFieldDelegate()
        sut.delegate = inspector
        ...
        dependencies.register(for: AmountFormatterProvider.self) { _ in
            return CoreCurrencyFormatterProvider()
        }
...
class AmountTextFieldFormatterTwoDecimalsTests: XCTestCase {
    ...
    override func setUp() {
        sut = AmountTextFieldFormatter(maximumIntegerDigits: 12, maximumFractionDigits: 2)
        inspector = MockChangeTestFieldDelegate()
        sut.delegate = inspector
        textField = UITextField()
        textField.delegate = sut
