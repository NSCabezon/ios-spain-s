...
class DynamicTooltipViewController: UIViewController {
    ...
    private let titleFont = UIFont.latoRegular(size: 14)
    ...
    func prepareLoader() {
        loader.setSecondaryLoader(scale: 2.0)
        loaderContainer.isHidden = true
    }
    
    public func showLoading(show: Bool) {
        loaderContainer.isHidden = !show
        show ? loader.restartIfNecessary() : loader.removeLoader()
    }
    
    public func setTooltipTitleAndDescription(title: LocalizedStylableText, description: String) {
        popoverTitle = title
