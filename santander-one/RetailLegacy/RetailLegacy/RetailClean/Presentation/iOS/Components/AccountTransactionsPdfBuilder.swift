...
class AccountTransactionsPdfBuilder {
    ...
    private func addNewTableHeader() {
        ...
        <th class="cell r-al s-op">
        <div class="cellHeaderContent">
        <span th:text="${t8}">\(stringLoader.getString("pdf_label_balance").text)</span>
        <svg class="fondoHeader">
        <rect width="100%" height="100%" style="fill:#eaeaea !important"/>
        </svg>
        </div>
    ...
    func addTransactionsInfo(_ info: [AccountTransaction]) {
            ...
            <tr th:each="statement : ${statements}">
                <td class="cell l-al f-op">
                    <div class="cellTableContent">
                        <span th:text="${statement.p2}">\(timeManager.toString(date: info[i].operationDate, outputFormat: .dd_MMM_yyyy) ?? "")</span>
                        <div class="dateContainer fValue">
                            <span th:text="${t6}">\(stringLoader.getString("pdf_label_valueDate", [StringPlaceholder(.date, timeManager.toString(date: info[i].valueDate, outputFormat: .dd_MMM_yyyy) ?? "")]).text)</span>
                ...
                <td class="cell l-al c-op">
                    <div class="cellTableContent">
                        <span th:text="${statement.p5}">\(info[i].description.camelCasedString)</span>
                    </div>
                </td>
                <td class="cell r-al i-op">
                    <div class="cellTableContent">
                        <span th:text="${statement.p7}" class="quantity">\(info[i].amount.getFormattedAmountUI())</span>
                    </div>
                </td>
                <td class="cell r-al s-op">
                    <div class="cellTableContent">
                        <span th:text="${statement.p8}" class="quantity">\(info[i].balance.getFormattedAmountUI())</span>
                    </div>
                </td>
            </tr>
            """
    ...
    func addDisclaimer() {
        ...
        <section class="disclaimer">
            <p th:text="${t7}">\(stringLoader.getString("pdf_text_lastMovements").text)</p>
        </section>
        """
    }
