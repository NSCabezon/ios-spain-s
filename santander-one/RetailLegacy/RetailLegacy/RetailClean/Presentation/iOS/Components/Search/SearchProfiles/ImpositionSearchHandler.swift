...
class ImpositionSearchHandler: ProductSearchParametersHandler<Imposition, DateRangeSearchParameters> {
    ...
    override func useCaseForProductTransactions<Input, Response, Error>(product: Imposition, pagination: PaginationDO?) -> UseCase<Input, Response, Error>? where Error: StringErrorOutput {
        ...
        let input = GetImpositionsTransactionUseCaseInput(imposition: product, dateFilter: dateFilter, pagination: pagination)
        
        return dependencies.useCaseProvider.getImpositionsTransactionsUseCase(input: input) as? UseCase<Input, Response, Error>
    }

    // MARK: - TrackerScreenProtocol

    override var screenId: String? {
        return TrackerPagePrivate.DepositsImpositionsSearch().page
    }
