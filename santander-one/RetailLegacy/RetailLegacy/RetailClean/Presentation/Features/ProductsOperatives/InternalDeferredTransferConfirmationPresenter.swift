...
class InternalDeferredTransferConfirmationPresenter: OperativeStepPresenter<InternalTransferConfirmationViewController, VoidNavigator, InternalTransferConfirmationPresenterProtocol> {
    ...
    func periodicity(_ periodicity: OnePayTransferTime) -> ConfirmationTableViewItemModel {
        let periodicity: String
        let parameter: InternalTransferOperativeData = containerParameter()
        switch parameter.time {
        case .day?:
            periodicity = dependencies.stringLoader.getString("confirmation_label_delayed").text
        ...
        case .now?:
            periodicity = dependencies.stringLoader.getString("confirmation_item_timely").text
        default:
    ...
    private func addConfirmation(of internalTransfer: InternalTransfer, to confirmationSection: TableModelViewSection) {
        ...
        if case let .day(newDate) = time {
            issueDate = newDate
        } else {
            issueDate = transferAccount.issueDate
        }
        
        let items = [
            periodicity(time),
            ConfirmationTableViewItemModel(stringLoader.getString("confirmation_item_issuanceDate"), dependencies.timeManager.toString(date: issueDate, outputFormat: TimeFormat.d_MMM_yyyy) ?? "", false, dependencies, .normal),
...
extension InternalDeferredTransferConfirmationPresenter: InternalTransferConfirmationPresenterProtocol {
