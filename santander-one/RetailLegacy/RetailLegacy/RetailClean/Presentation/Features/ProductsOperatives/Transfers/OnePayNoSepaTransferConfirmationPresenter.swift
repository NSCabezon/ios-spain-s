...
class OnePayNoSepaTransferConfirmationPresenter: OperativeStepPresenter<TransferConfirmationNoSepaViewController, Void, TransferConfirmationNSPresenterProtocol> {
    
    // MARK: - Tracks
    override var screenId: String? {
        return TrackerPagePrivate.NoSepaTransferConfirmation().page
    }
    ...
    override func loadViewData() {
        ...
        addAccount(operativeData.account, to: accountSection)
    ...
    private func addConfirmation(to confirmationSection: TableModelViewSection) {
        ...
        confirmationBuilder.add("confirmation_item_beneficiary", string: operativeData.beneficiary)
        if let beneficiaryAddress = operativeData.beneficiaryAddress?.address, !beneficiaryAddress.isEmpty {
            confirmationBuilder.add("confirmation_label_beneficiaryAddress", string: beneficiaryAddress)
        }
        if let beneficiaryAddressLocality = operativeData.beneficiaryAddress?.locality, !beneficiaryAddressLocality.isEmpty {
            confirmationBuilder.add("confirmation_label_beneficiaryTown", string: beneficiaryAddressLocality)
        }
        
        if let beneficiaryAddressCountry = operativeData.beneficiaryAddress?.country,
            !beneficiaryAddressCountry.isEmpty,
            !operativeData.isSepaNoEur {
            confirmationBuilder.add("confirmation_label_beneficiaryCountry", string: beneficiaryAddressCountry)
        }
        ...
        confirmationBuilder.add(titleDestinationCountry, string: operativeData.country.name)
        ...
        confirmationBuilder.addExchangeType("confirmation_item_exchangeRate", data: operativeData)
        ...
        confirmationSection.add(item: ConfirmationTextFieldViewModel(inputIdentifier: Constants.email.rawValue, title: dependencies.stringLoader.getString("confirmation_label_sentMail"), placeholder: dependencies.stringLoader.getString("confirmation_label_optional"), keyboardType: .emailAddress, privateComponent: dependencies))
    ...
    private func addTransferInfo(to section: TableModelViewSection) {
        let operativeData: NoSepaTransferOperativeData = containerParameter()
        guard let destinationAccount = operativeData.beneficiaryAccount?.account else { return }
        let transferAmount = operativeData.amount.getAbsFormattedAmountUI()
...
extension OnePayNoSepaTransferConfirmationPresenter: TransferConfirmationNSPresenterProtocol {
    
    func onContinueButtonClicked() {
        let operativeData: NoSepaTransferOperativeData = containerParameter()
        let beneficiaryAddress = operativeData.beneficiaryAddress
        let swiftValidation = operativeData.swiftValidation
        guard let beneficiary = operativeData.beneficiary, let beneficiaryAccount = operativeData.beneficiaryAccount else { return }
        ...
        let input = ValidateNoSepaTransferUseCaseInput(
            ...
            beneficiaryEmail: dataEntered(for: Constants.email.rawValue)
        )
