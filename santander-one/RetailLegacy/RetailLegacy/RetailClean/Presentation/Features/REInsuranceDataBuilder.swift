...
struct REInsuranceDataBuilder {
    ...
    func getDataForREInsurance(_ insuranceData: InsuranceData, insuranceSaving: InsuranceSaving) -> [DateProvider] {
        ...
        if let totalAmount = insuranceData.totalAmount {
            data.append(InsuranceGeneralDataViewModel(isFirst: false, isLast: false, isCopiable: false, infoTitle: dependencies.stringLoader.getString("insurances_label_totalAmountPeriodic"), info: totalAmount.getFormattedAmountUIWith1M(), privateComponent: dependencies, shareDelegate: shareDelegate))
        }
        
        if let installmentAccountId = insuranceData.installmentAccountId {
            data.append(InsuranceGeneralDataViewModel(isFirst: false, isLast: false, isCopiable: false, infoTitle: dependencies.stringLoader.getString("insurances_label_installmentAccountId"), info: installmentAccountId, privateComponent: dependencies, shareDelegate: shareDelegate))
        }
        
        if let insuredAmount = insuranceData.insuredAmount {
            data.append(InsuranceGeneralDataViewModel(isFirst: false, isLast: true, isCopiable: false, infoTitle: dependencies.stringLoader.getString("insurances_label_insuredAmount"), info: insuredAmount.getFormattedAmountUIWith1M(), privateComponent: dependencies, shareDelegate: shareDelegate))
        }
    ...
    func getParticipantsDataForREInsurance(_ participants: InsuranceParticipantList) -> [DateProvider] {
        var result = [DateProvider]()
    ...
    func getBeneficiariesDataForREInsurance(_ beneficiaries: InsuranceBeneficiaryList) -> [DateProvider] {
        var result = [DateProvider]()
