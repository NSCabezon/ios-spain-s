//
//  SavingsTransactionsPdfBuilder.swift
//  RetailLegacy
//
...
class SavingsTransactionsPdfBuilder {
    ...
    func addTransactionsInfo(_ info: [SavingTransactionRepresentable]) {
        ...
        for i in 0..<info.count {
            ...
            let description = info[i].transactionInformation ?? ""
            html += """
            <tr th:each="statement : ${statements}">
                <td class="cell l-al f-op">
                    <div class="cellTableContent">
                        <span th:text="${statement.p2}">\(timeManager.toString(date: info[i].bookingDateTime, outputFormat: .dd_MM_yyyy) ?? "")</span>
                        <div class="dateContainer fValue">
                            <span th:text="${t6}">\(stringLoader.getString("pdf_label_valueDate", [StringPlaceholder(.date, timeManager.toString(date: info[i].valueDateTime, outputFormat: .dd_MMM_yyyy) ?? "")]).text)</span>
                ...
                <td class="cell r-al i-op">
                    <div class="cellTableContent">
                        <span th:text="${statement.p7}" class="quantity">\(info[i].amount.amount) \(info[i].amount.currency)</span>
                    </div>
                </td>
            </tr>
            """
    ...
    func addDisclaimer() {
        ...
        <section class="disclaimer">
            <p th:text="${t7}">\(stringLoader.getString("pdf_text_legalFooter").text.replacingOccurrences(of: "\n", with: "<br/><br/>"))</p>
        </section>
        """
    }
