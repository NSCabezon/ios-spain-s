...
@IBDesignable class RangeSlider: UIControl {
    ...
    convenience init(frame: CGRect = .zero, completion: ((RangeSlider) -> Void)? = nil) {
    ...
    @IBInspectable var selectedMinValue: CGFloat = 0.0 {
        didSet {
    ...
    @IBInspectable var selectedMaxValue: CGFloat = 100.0 {
        didSet {
            if selectedMaxValue > maxValue {
                selectedMaxValue = maxValue
            }
            if selectedMaxValue < minValue {
                selectedMaxValue = minValue
            }
    ...
    @IBInspectable var labelsFixed: Bool = false

    /// The minimum distance the two selected slider values must be apart. Default is 0.
    @IBInspectable var minDistance: CGFloat = 0.0 {
        didSet {
            if minDistance < 0.0 {
                minDistance = 0.0
            }
    ...
    @IBInspectable var maxDistance: CGFloat = .greatestFiniteMagnitude {
        didSet {
            if maxDistance < 0.0 {
    ...
    @IBInspectable var handleBorderColor: UIColor?

    /// Set slider line tint color between handles
    @IBInspectable var colorBetweenHandles: UIColor?
