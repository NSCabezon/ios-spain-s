...
// MARK: Usual Transfer
    ...
    public func loadUsualTransfersOld() throws -> BSANResponse<Void> {
        ...
        if meta.isOK() {
            ...
            if var transferDTOs = response.transferDTOs {
                for i in 0...transferDTOs.count-1 {
                    transferDTOs[i].transferAmount = nil
                    transferDTOs[i].concept = nil
                }
                bsanDataProvider.storeUsualTransfers(transferDTO: transferDTOs)
            }
    ...
    public func loadUsualTransfers() throws -> BSANResponse<Void> {
        ...
        if (meta.isOK()) {
            ...
            if let transfers = response.transferDTOs {
                transfersDTOs.append(contentsOf: transfers)
            }
            ...
            for var transfer in transfersDTOs {
                transfer.transferAmount = nil
                transfer.concept = nil
            }
            bsanDataProvider.storeUsualTransfers(transferDTO: transfersDTOs)
    ...
    public func validateUpdateSepaPayee(payeeId: String?, transferDTO: TransferDTO?, newCurrencyDTO: CurrencyDTO?, newBeneficiaryBAOName: String?, newIban: IBANDTO?) throws -> BSANResponse<SignatureWithTokenDTO> {
        guard let transferDTO = transferDTO else { return BSANErrorResponse(nil) }
        let bsanAssemble = BSANAssembleProvider.getSepaPayeeDetailAssemble()
        let bsanEnvironment: BSANEnvironmentDTO = try bsanDataProvider.getEnvironment()
        let authCredentials = try bsanDataProvider.getAuthCredentials()
        let userDataDTO = try bsanDataProvider.getUserData()
    ...
    public func getUsualTransfers() throws -> BSANResponse<[TransferDTO]>{
        return BSANOkResponse(try bsanDataProvider.get(\.transferInfo).usualTransfersList)
    }
    
    public func validateUsualTransfer(originAccountDTO: AccountDTO, usualTransferInput: UsualTransferInput, transferDTO: TransferDTO) throws -> BSANResponse<ValidateAccountTransferDTO> {
        ...
        let request = ValidateUsualTransferRequest(
            BSANAssembleProvider.getTransfersAssemble(try bsanDataProvider.isPB()),
            try bsanDataProvider.getEnvironment().urlBase,
            ValidateUsualTransferRequestParams(token: authCredentials.soapTokenCredential,
                                               userDataDTO: userDataDTO,
                                               version: bsanHeaderData.version,
                                               terminalId: bsanHeaderData.terminalID,
                                               language: bsanHeaderData.language,
                                               beneficiary: transferDTO.beneficiary ?? "",
                                               ibandto: transferDTO.iban,
    ...
    public func confirmUsualTransfer(originAccountDTO: AccountDTO, usualTransferInput: UsualTransferInput, transferDTO: TransferDTO, signatureDTO: SignatureDTO) throws -> BSANResponse<TransferConfirmAccountDTO> {
        ...
        let request = ConfirmUsualTransferRequest(
            BSANAssembleProvider.getTransfersAssemble(try bsanDataProvider.isPB()),
            try bsanDataProvider.getEnvironment().urlBase,
            ConfirmUsualTransferRequestParams(token: authCredentials.soapTokenCredential,
                                              userDataDTO: userDataDTO,
                                              version: bsanHeaderData.version,
                                              terminalId: bsanHeaderData.terminalID,
                                              language: bsanHeaderData.language,
...
// MARK: Generic Scheduled Transfers
    ...
    public func storeGetHistoricalTransferCompleted(_ completed: Bool) throws {
        try self.bsanDataProvider.storeGetHistoricalTransferCompleted(completed)
    }
...
// MARK: Emitted Transfers
    ...
    public func getEmittedTransfers() throws -> BSANResponse<[String : TransferEmittedListDTO]>{
        return BSANOkResponse(try bsanDataProvider.get(\.transferInfo).transfersEmittedDictionary)
    }
    
    public func loadEmittedTransfers(account: AccountDTO, amountFrom: AmountDTO?, amountTo: AmountDTO?, dateFilter: DateFilter, pagination: PaginationDTO?) throws -> BSANResponse<TransferEmittedListDTO> {
        let authCredentials = try bsanDataProvider.getAuthCredentials()
        let userDataDTO = try bsanDataProvider.getUserData()
        let bsanHeaderData = try bsanDataProvider.getBsanHeaderData()
        let newGlobalPosition: GlobalPositionDTO = try bsanDataProvider.get(\.newGlobalPositionDTO)
        let requestAccount: AccountDTO = newGlobalPosition.accounts?.first(where: { $0.iban == account.iban }) ?? account
        if let contract: ContractDTO = requestAccount.contract {
            let contractKey: String = contract.contratoPK
            let transferEmittedListDTO = try bsanDataProvider.get(\.transferInfo).transfersEmittedDictionary[contractKey]
            ...
            let request = GetEmittedTransfersRequest(
                ...
                try bsanDataProvider.getEnvironment().urlBase,
                GetEmittedTransferRequestParams(token: authCredentials.soapTokenCredential,
                                                userDataDTO: userDataDTO,
                                                dialectISO: bsanHeaderData.dialectISO,
                                                languageISO: bsanHeaderData.languageISO,
            ...
            if (meta.isOK()) {
                ...
                if let result = try bsanDataProvider.get(\.transferInfo).transfersEmittedDictionary[contractKey] {
                    return BSANOkResponse(result)
                }
            }
            return BSANErrorResponse(meta)
...
// MARK: Generic Transfer
    ...
    public func confirmGenericTransfer(originAccountDTO: AccountDTO, nationalTransferInput: GenericTransferInputDTO, otpValidationDTO: OTPValidationDTO?, otpCode: String?) throws -> BSANResponse<TransferConfirmAccountDTO> {
        return try confirmGenericTransfer(
    ...
    public func loadAllUsualTransfers() throws -> BSANResponse<[TransferDTO]> {
        let authCredentials = try bsanDataProvider.getAuthCredentials()
        let userDataDTO = try bsanDataProvider.getUserData()
        let bsanHeaderData = try bsanDataProvider.getBsanHeaderData()
        ...
        if (meta.isOK()) {
            ...
            return BSANOkResponse(meta, transfersDTOs)
        }
