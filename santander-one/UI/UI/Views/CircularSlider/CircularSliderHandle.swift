...
public enum CircularSliderHandleType: Int, RawRepresentable {
    case smallCircle = 0,
    mediumCircle,
    largeCircle,
...
public class CircularSliderHandle: CALayer {
    ...
    private var slider: CircularSlider {
        return delegate as? CircularSlider ?? CircularSlider()
    }
    ...
    internal var handleType: CircularSliderHandleType = .largeCircle {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    private func imageRotated(img: UIImage, byDegrees degrees: CGFloat) -> UIImage {
        ...
        let transform = CGAffineTransform(rotationAngle: CGFloat(toRad(Double(degrees))))
