//
//  GifImageDecoder.swift
//  UI
//
//  Created by José Carlos Estela Anguita on 18/2/22.
//
...
final class GifImageDecoder {
    ...
    struct DecodingOptions {
        ...
        static var standard: DecodingOptions {
            return DecodingOptions(mode: .asynchronous, sizeForDrawing: nil)
        }
    ...
    enum DownSamplingLevel: Int {
        ...
        public static var standard: DownSamplingLevel {
            return .level0
        }
    }
    ...
    func setDataProvider(_ dataProvider: CGDataProvider) {
        CGImageSourceUpdateDataProvider(imageSource, dataProvider, true)
    }
    ...
    func frameDuration(at index: Int) -> TimeInterval? {
        ...
        var animationProperties = GifImageDecoder.animationProperties(from: frameProperties)

        if animationProperties == nil {
            if let properties = CGImageSourceCopyProperties(imageSource, imageSourceOptions()) as? [CFString: Any] {
                animationProperties = GifImageDecoder.animationHEICSProperties(from: properties, at: index)
            }
        }
    ...
    func frameSize(at index: Int, downsamplingLevel: DownSamplingLevel = .standard) -> CGSize? {
    ...
    func createFrameImage(at index: Int, downsamplingLevel: DownSamplingLevel = .standard, decodingOptions: DecodingOptions = .standard) -> CGImage? {
    ...
    private func imageSourceOptions(with downsamplingLevel: DownSamplingLevel = .standard) -> CFDictionary {
        var options = GifImageDecoder.imageSourceOptions
    ...
    private func imageSourceAsyncOptions(sizeForDrawing: CGSize, donwsamplingLevel: DownSamplingLevel = .standard) -> CFDictionary {
        var options = GifImageDecoder.imageSourceAsyncOptions
...
extension GifImageDecoder {
    ...
    var animatedUIImage: UIImage? {
        ...
        for index in 0 ..< frameCount {
            guard let image = createFrameUIImage(at: index) else {
                continue
            }

            images.append(image)
            duration += frameDuration(at: index) ?? 0.0
        }
    ...
    private func createFrameUIImage(at index: Int, downsamplingLevel: DownSamplingLevel = .standard, decodingOptions: DecodingOptions = .standard) -> UIImage? {
