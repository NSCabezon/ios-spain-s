...
extension SecIdentity
{
     ...
     */
    public static func create(ofSize bits:UInt = 3072, subjectCommonName name:String, subjectOrganizationName organization:String, contryName country: String) -> SecIdentity? {
        let privKey: SecKey
        let pubKey: SecKey
        do {
        ...
        let certRequest = CertificateRequest(forPublicKey:pubKey, subjectCommonName: name, subjectOrganizationName: organization, countryName: country, keyUsage: [.DigitalSignature, .DataEncipherment])

        guard let signedBytes = certRequest.selfSign(withPrivateKey:privKey) else {
            return nil
        }
        
        guard let signedCert = SecCertificateCreateWithData(nil, Data(signedBytes) as CFData) else {
            return nil
        }
