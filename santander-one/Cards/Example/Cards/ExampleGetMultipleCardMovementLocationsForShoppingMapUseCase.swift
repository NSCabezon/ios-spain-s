//
//  ExamopleGetMultipleCardMovementLocationsForShoppingMapUseCase.swift
//  Cards_Example
//
...
struct ExampleGetMultipleCardMovementLocationsForShoppingMapUseCase {
...
extension ExampleGetMultipleCardMovementLocationsForShoppingMapUseCase: GetMultipleCardMovementLocationsForShoppingMapUseCase {
    func fetchLocationsForShoppingMap(card: CardRepresentable) -> AnyPublisher<[CardMovementLocationRepresentable], Never> {
        let output = getOutput(card: card)
        return Just(output)
            .eraseToAnyPublisher()
    }
}

private extension ExampleGetMultipleCardMovementLocationsForShoppingMapUseCase {
    func getOutput(card: CardRepresentable) -> [CardMovementLocationRepresentable] {
        var output = [CardMovementLocationRepresentable]()
        do {
            let cardEntity = CardEntity(card)
            let locationsListResponse = try cardsManager.getCardTransactionLocationsList(card: cardEntity.dto)
