...
final class TransferPDFBuilder {
    ...
    func addHeader(title: String, office: String?, date: Date?) {
        ...
        <p class="txt_rotar">
        \(localized("pdf_text_datesBank").text)
        </p>
        </div>
        ...
        <section>
        <h1>\(localized(title).text)</h1>
        </section>
        <section class="txt_right">
        """
        if let office = office {
            html += """
            <p><span>\(localized("pdf_label_office").text)</span><br>
            <span>\(office)</span><br>
            """
        }
        
        if let dateText = dateToString(date: date, outputFormat: .dd_MMM_yyyy) {
            html += "<span>\(localized("pdf_label_deliveryDate").text)</span> <span>\(dateText)</span></p>"
        }
        html += "</section></div>"
        html += "<table width=\"82%\">"
    }
    ...
    func addAccounts(originAccountAlias: String, originAccountIBAN: String, destinationAccountAlias: String, destinationAccountIBAN: String) {
        ...
        <span style="display:block;">\(localized("pdf_label_of").text)</span>
        ...
        <td style="width:49.5%; background-color: #F5F7FA;">
        <div style="margin: 10px">
        <span style="display:block;">\(localized("pdf_label_to").text)</span>
    ...
    func addAccounts(originAccount: AccountEntity, destinationAccount: AccountEntity) {
        addAccounts(originAccountAlias: originAccount.alias ?? "",
                    originAccountIBAN: originAccount.getIBANShort,
                    destinationAccountAlias: destinationAccount.alias ?? "",
                    destinationAccountIBAN: destinationAccount.getIBANShort)
    }
    ...
    func build() -> String {
        ...
        <p>\(localized("pdf_text_seePdf").text)</p>
        </footer>
        </div>
        </section>
        </body>
    ...
    private func htmlCell(for info: PDFInfo, withPercentage percentage: Float) -> String {
        ...
        <div style="margin: 10px">
        <span style="display:block;">\(localized(info.key).text)</span>
