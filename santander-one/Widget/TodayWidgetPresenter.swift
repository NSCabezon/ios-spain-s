...
class TodayWidgetPresenter {
    ...
    private lazy var localeManager: TimeManager & StringLoader = {
        return WidgetDependencies.localeManger
    }()
    ...
    private func doPg(isPb: Bool) {
        ...
        UseCaseWrapper(with: usecase, useCaseHandler: WidgetDependencies.usecaseHandler, errorHandler: nil, includeAllExceptions: true, onSuccess: { [weak self] response in
            switch response {
            case .pg(let amount, let accounts):
    ...
    private func checkUser(name: String, type: String, completion: @escaping (_ response: WidgetUserUseCaseOkOutput) -> Void) {
        ...
        UseCaseWrapper(with: usecase, useCaseHandler: WidgetDependencies.usecaseHandler, errorHandler: nil, includeAllExceptions: true, onSuccess: {
            response in
            completion(response)
        }, onError: { _ in
...
extension TodayWidgetPresenter: WidgetAccountInfoDataProviderProtocol {
    ...
    func rows() -> [WidgetRowInfo] {
        return lastDataSnapshot?.transactions?.map({
            ...
            case let .message(key):
                let text = localeManager.getString(key).plainText
            ...
            case let .transaction(date: date, title: title, amount: amount):
                ...
                var month: String?
                if let date = date {
                    day = timeManager.toString(date: date, outputFormat: .d)?.uppercased()
                    month = timeManager.toString(date: date, outputFormat: .MMM)?.uppercased()
                }
    ...
    var accountTitle: String? {
        return  localeManager.getString("widget_label_balance").plainText
    }
    ...
    var currentState: String? {
        return localeManager.getString("widget_label_update_ios", [StringPlaceholder(.date, lastUpdate ?? "")]).plainText
    }
    
    private var lastUpdate: String? {
        return timeManager.toString(date: lastDataSnapshot?.lastUpdate, outputFormat: .dd_MMM_yyyy_HHmm, timeZone: .local)
    }
}
...
extension TodayWidgetPresenter: TodayWidgetPresenterProtocol {
    ...
    var deeplinkOptions: [WidgetButtonData] {
        return DeeplinkOption.allCases.map({(localeManager.getString($0.data.0).plainText, $0.data.1)})
    }
...
extension TodayWidgetPresenter: RefreshCapable {
    ...
    var refresh: RefreshType {
        guard case .transactionList(_) = currentSection else {
...
extension TodayWidgetPresenter: WidgetMessageLabelDataProviderProtocol {
    ...
    var message: String? {
        ...
        return localeManager.getString(key).plainText
    }
