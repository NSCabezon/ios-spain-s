...
class WidgetAccountInfoView: UIView {
...
extension WidgetAccountInfoView: ViewCreatable {}
...
extension WidgetAccountInfoView: Refreshable {
    ...
    func refreshData() {
        titleLabel.text = dataProvider?.accountTitle
        subTitleLabel.text = dataProvider?.amountAvailable
        subTitleLabel.scaleDecimals()
        stateLabel.text = dataProvider?.currentState
        for v in listStackView.arrangedSubviews {
            listStackView.removeArrangedSubview(v)
        ...
        for row in rows {
            ...
            case let .title(title):
                guard let view = WidgetTitleRowView.instantiateFromNib() else { return }
                view.setTitle(title)
                listStackView.addArrangedSubview(view)
            case let .transaction(day: day, month: month, title: title, amount: amount):
                guard let view = WidgetTransactionRowView.instantiateFromNib() else { return }
                view.setDate(day: day, month: month)
                view.setTitle(title)
                view.setAmount(amount)
                listStackView.addArrangedSubview(view)
            case let .message(text):
                guard let view = WidgetMessageRowView.instantiateFromNib() else { return }
                view.setMessage(text)
                listStackView.addArrangedSubview(view)
            case let .lineMessage(text):
                guard let view = WidgetLineMessageRowView.instantiateFromNib() else { return }
                view.setMessage(text)
                listStackView.addArrangedSubview(view)
            }
        }
