...
final class TransferPDFBuilder {
    ...
    func addAccounts(originAccount: AccountEntity, destinationAccount: AccountEntity, dependenciesResolver: DependenciesResolver) {
        if let accountNumberFotmat = dependenciesResolver.resolve(forOptionalType: AccountNumberFormatterProtocol.self) {
            let originAccountNumber = accountNumberFotmat.accountNumberFormat(originAccount)
            let destinationAccountNumber = accountNumberFotmat.accountNumberFormat(destinationAccount)
            addAccounts(originAccountAlias: originAccount.alias ?? "", originAccountIBAN: originAccountNumber, destinationAccountAlias: destinationAccount.alias ?? "", destinationAccountIBAN: destinationAccountNumber)
        } else {
            addAccounts(originAccountAlias: originAccount.alias ?? "", originAccountIBAN: originAccount.getIBANShort, destinationAccountAlias: destinationAccount.alias ?? "", destinationAccountIBAN: destinationAccount.getIBANShort)
        }
    }
...
private extension TransferPDFBuilder {
    ...
    func add(info: [PDFInfo], numberOfItemsInRow itemsInRow: Int, isFirst: Bool = false) {
    ...
    func htmlCell(for info: PDFInfo, withPercentage percentage: Float) -> String {
        return """
    ...
    func emptyCell(withPercentage percentage: Float) -> String {
        return """
        <td style="width:\(percentage)%; display: inline-table;">
        <div style="margin: 10px"/>
        </td>
    ...
    func pdfInfo(from info: (String, PDFStringConvertible?)) -> PDFInfo? {
