import Foundation

public class GetScheduledTransfersRequest: BSANSoapRequest<GetScheduledTransfersRequestParams, GetScheduledTransfersHandler, GetScheduledTransfersRespose, GetScheduledTransfersParser> {
    
    private static let SERVICE_NAME = "listaPeriodicasDiferidasLa"
    
    override var nameSpace: String {
        return "http://www.isban.es/webservices/TRASAN/Periodicas_la/F_trasan_periodicas_la/"
    }
    
    public override var serviceName: String {
        return GetScheduledTransfersRequest.SERVICE_NAME
    }
    
    override var message: String {
        
        let pagination = params.paginationDTO?.repositionXML ?? getEmptyPagination()
        let amountFrom = params.amountFrom ?? AmountDTO()
        let amountTo = params.amountTo ?? AmountDTO()
        
        return "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" " +
            "   xmlns:v1=\"\(nameSpace)\(facade)/v1\">" +
            "   <soapenv:Header>\(getSecurityHeader(params.token))</soapenv:Header>" +
            "   <soapenv:Body>" +
            "      <v1:\(serviceName) facade=\"\(facade)\">" +
            "        <datosConexion>" +
            "            <idioma>" +
            "               <IDIOMA_ISO>\(serviceLanguage(params.languageISO))</IDIOMA_ISO>" +
            "               <DIALECTO_ISO>\(params.dialectISO)</DIALECTO_ISO>" +
            "            </idioma>" +
            "                \(params.userDataDTO.getUserDataWithChannelAndCompanyAndMultiContract)" +
            "        </datosConexion>" +
            "             <entrada>" +
            "              <importeDesde>" +
            "               <IMPORTE>\(amountFrom.getDecimalPart())</IMPORTE>" +
            "               <DIVISA>\(amountFrom.currency?.currencyName ?? "")</DIVISA>" +
            "            </importeDesde>" +
            "            <importeHasta>" +
            "               <IMPORTE>\(amountTo.getDecimalPart())</IMPORTE>" +
            "               <DIVISA>\(amountTo.currency?.currencyName ?? "")</DIVISA>" +
            "            </importeHasta>" +
            "            <divisa>\(params.currency)</divisa>" +
            "            <iban>" +
            "                <PAIS>\(params.iban.countryCode)</PAIS>" +
            "                <DIGITO_DE_CONTROL>\(params.iban.checkDigits)</DIGITO_DE_CONTROL>" +
            "                <CODBBAN>\(params.iban.codBban30)</CODBBAN>" +
            "            </iban>" +
            "          \(pagination)" +
            "        </entrada>" +
            "        </v1:\(serviceName)>" +
            "   </soapenv:Body>" +
            "</soapenv:Envelope>";
    }
    
    private func getEmptyPagination() -> String {
        return "<repos>" +
            "            <indMasDatTrfInt/>" +
            "            <fecEjecTrfInt/>" +
            "            <impTrfInt>" +
            "               <IMPORTE/>" +
            "               <DIVISA/>" +
            "            </impTrfInt>" +
            "            <ordSerTrfInt>" +
            "               <CENTRO>" +
            "                  <EMPRESA/>" +
            "                  <CENTRO/>" +
            "               </CENTRO>" +
            "               <PRODUCTO/>" +
            "               <NUMERO_DE_ORDEN/>" +
            "            </ordSerTrfInt>" +
            "            <indRutInterMisPag/>" +
            "            <indMasDatMisPag/>" +
            "            <NaturPagoMisPag>" +
            "                 <EMPRESA/>" +
            "                 <COD_ALFANUM/>" +
            "            </NaturPagoMisPag>" +
            "            <fecEjecMisPag/>" +
            "            <impMisPag>" +
            "               <IMPORTE/>" +
            "               <DIVISA/>" +
            "            </impMisPag>" +
            "            <NumOrdCabMisPag/>" +
            "</repos>"
    }

}

public struct GetScheduledTransfersRequestParams {
    public let token: String
    public let userDataDTO: UserDataDTO
    public let dialectISO: String
    public let languageISO: String
    public let iban: IBANDTO
    public let amountFrom: AmountDTO?
    public let amountTo: AmountDTO?
    public let currency: String
    public let paginationDTO: PaginationDTO?
    
}
