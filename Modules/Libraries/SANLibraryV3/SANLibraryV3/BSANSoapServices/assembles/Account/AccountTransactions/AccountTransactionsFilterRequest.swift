public class AccountTransactionsFilterRequest: BSANSoapRequest <AccountTransactionsRequestParams, AccountTransactionsHandler, AccountTransactionsResponse, AccountTransactionsParser> {
    ...
    override var message: String {
        ...
        let returnString = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" " +
                ...
                "               </datosCabecera>" +
                "               <importeDesde>\(AmountFormats.getValueForWS(value: params.filter?.startAmount?.value))</importeDesde>" +
                "               <importeHasta>\(AmountFormats.getValueForWS(value: params.filter?.endAmount?.value))</importeHasta>" +
                ...
                "               <conceptoConsAv>\(params.filter?.transferType.code ?? "")</conceptoConsAv>" +
                "               <tipoOrdenacion>D</tipoOrdenacion>" +
                "               <tipoMovimiento>\(params.filter?.movementType.code ?? "")</tipoMovimiento>" +
                "               \(pagination ?? "")" +
                "           </entrada>" +
                "       </v1:\(serviceName)>" +
                "   </soapenv:Body>" +
