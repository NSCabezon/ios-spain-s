//
//  EcommerceGetTokenPushUseCaseTest.swift
...
class EcommerceGetTokenPushUseCaseTest: XCTestCase {
    ...
    func test_useCase_withRequestValues_shouldBeOk() {
        ...
        do {
            let response = try getEcommerceTokenPushUseCase(fakeRequestValues)
            XCTAssert(response.isOkResult)
        } catch {
            XCTFail("GetEcommerceTokenPushUseCase: throws")
        }
    }
...
private extension EcommerceGetTokenPushUseCaseTest {
    // MARK: - Setup
    func setTests() {
        setDependencies()
        setQuickSetup()
    ...
    // MARK: - Prepare test
    func getEcommerceTokenPushUseCase(_ requestValues: Void) throws -> UseCaseResponse<GetEcommercePushNotificationUseCaseOkOutput, StringErrorOutput> {
